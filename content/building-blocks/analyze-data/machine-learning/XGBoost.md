...
{{% warning %}}
Because XGBoost only works with numeric vectors, so you need to vectors, so you need to convert the categorical values into numerical ones using one-hot encoding or any other approaches.
{{%/ warning %}}
