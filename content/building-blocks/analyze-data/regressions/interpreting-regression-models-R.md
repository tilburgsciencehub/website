...
We can apply the same logic from the Linear-Log model, but now a 1-unit change in X is assoicated with a $\beta_1$-unit change in ln(Y), which then means a $\beta_1 \cdot 100 \percent$ change in Y. 
